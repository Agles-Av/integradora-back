...
public class RespuestaUsuarioService {
    ...
    @Transactional(rollbackFor = {SQLException.class})
    public ResponseEntity<ApiResponse> chageCorrectaRespuesta(Long id){
        Optional<RespuestaUsuario> foundRespuestaUsu = repository.findById(id);
        if (foundRespuestaUsu.isEmpty())
            return new ResponseEntity<>(new ApiResponse(
                    HttpStatus.NOT_FOUND,
        ...
        RespuestaUsuario respuestaUsuario = foundRespuestaUsu.get();
        respuestaUsuario.setCorrecta(!respuestaUsuario.isCorrecta());
        repository.save(respuestaUsuario);
        return new ResponseEntity<>(new ApiResponse(
                HttpStatus.OK,
                false,
